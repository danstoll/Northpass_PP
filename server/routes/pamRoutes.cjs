  ...
  try {
    ...
    for (const pam of pams) {
      try {
        ...
        const partners = await query(`
          SELECT
            p.id, p.account_name, p.partner_tier, p.account_region, p.total_npcu,
            (COALESCE(p.cert_count_nintex_ce, 0) + COALESCE(p.cert_count_nintex_k2, 0) +
             COALESCE(p.cert_count_nintex_salesforce, 0) + COALESCE(p.cert_count_go_to_market, 0)) as active_certs,
            p.cert_count_nintex_ce, p.cert_count_nintex_k2,
            p.cert_count_nintex_salesforce, p.cert_count_go_to_market, p.has_gtm_certification
          FROM partners p
          WHERE p.account_owner = ?
          ORDER BY p.account_name
        `, [pam.owner_name]);
        ...
        const expiringCerts = await query(`
          ...
          WHERE p.account_owner = ? AND e.status = 'completed' AND c.npcu_value > 0
            AND e.expires_at IS NOT NULL AND e.expires_at BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 90 DAY)
          ORDER BY e.expires_at LIMIT 20
        `, [pam.owner_name]);
              ...
              <tbody>
                ${partners.map(p => `
                  <tr>
                    ...
                    <td style="padding: 10px; text-align: center; border-bottom: 1px solid #eee; font-weight: 600; color: #FF6B35;">${p.total_npcu || 0}</td>
                    <td style="padding: 10px; text-align: center; border-bottom: 1px solid #eee;">${p.cert_count_nintex_ce || 0}</td>
                    <td style="padding: 10px; text-align: center; border-bottom: 1px solid #eee;">${p.cert_count_nintex_k2 || 0}</td>
                    <td style="padding: 10px; text-align: center; border-bottom: 1px solid #eee;">${p.cert_count_nintex_salesforce || 0}</td>
                    <td style="padding: 10px; text-align: center; border-bottom: 1px solid #eee;">${p.has_gtm_certification ? '✓' : '0'}</td>
                    <td style="padding: 10px; text-align: center; border-bottom: 1px solid #eee; font-weight: 600;">${p.active_certs || 0}</td>
                  </tr>
                `).join('')}
              </tbody>
        ...
        if (expiringCerts.length > 0) {
          expiringCertsSection = `
            <div style="background: #fff3cd; border-radius: 8px; padding: 20px; margin: 20px 0; border-left: 4px solid #856404;">
              <h3 style="margin: 0 0 15px; color: #856404;">Expiring Certifications (Next 90 Days)</h3>
              <table style="width: 100%; border-collapse: collapse;">
                <thead>
                  <tr>
                    <th style="padding: 8px; text-align: left; border-bottom: 1px solid #ddd;">User</th>
        ...
        // Build full email HTML
        const subject = `Partner Certification Report - ${reportDate}`;
        const emailHtml = `
          <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            <div style="background: linear-gradient(135deg, #6B4C9A, #FF6B35); padding: 20px; color: white; border-radius: 8px 8px 0 0;">
              <h1 style="margin: 0;">Partner Certification Report</h1>
            ...
            <div style="padding: 20px; background: white; border: 1px solid #ddd; border-top: none;">
              <p>Hello ${pam.owner_name.split(' ')[0]},</p>
              <p>Here's your weekly partner certification summary:</p>
              <h2 style="color: #6B4C9A; border-bottom: 2px solid #FF6B35; padding-bottom: 10px;">Your Partners (${partners.length})</h2>
              ${partnerTable}
              ${expiringCertsSection}
              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #ddd; color: #666; font-size: 12px;">
                <p>This report was generated automatically by the Nintex Partner Portal.</p>
        ...
        // Log the email
        await query(`
          INSERT INTO email_log (recipient_email, recipient_name, subject, email_type, status, pam_id, sent_at)
          VALUES (?, ?, ?, 'pam_report', 'sent', ?, NOW())
        `, [pam.email, pam.owner_name, subject, pam.id]);
        ...
        await query(`
          INSERT INTO email_log (recipient_email, recipient_name, subject, email_type, status, pam_id, error_message, sent_at)
          VALUES (?, ?, ?, 'pam_report', 'failed', ?, ?, NOW())
        `, [pam.email, pam.owner_name, 'Partner Certification Report', pam.id, err.message]).catch(() => {});
      }
    }
...
const executiveReportService = require('../db/executiveReportService.cjs');

// Get available users for executive report recipient selection
router.get('/executive-report/available-users', async (req, res) => {
  try {
...
router.get('/executive-report/recipients', async (req, res) => {
  try {
    const recipients = await executiveReportService.getRecipients();
    res.json({ recipients });
  } catch (error) {
    console.error('Error getting executive report recipients:', error);
    res.status(500).json({ error: error.message });
  }
});

// Add executive report recipient
router.post('/executive-report/recipients', async (req, res) => {
  try {
    const { email, name } = req.body;
    if (!email) {
      return res.status(400).json({ error: 'Email is required' });
    }
    await executiveReportService.addRecipient(email, name);
    const recipients = await executiveReportService.getRecipients();
    res.json({ success: true, recipients });
  } catch (error) {
    console.error('Error adding executive report recipient:', error);
    res.status(500).json({ error: error.message });
  }
});

// Remove executive report recipient
router.delete('/executive-report/recipients/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const success = await executiveReportService.removeRecipient(id);
    if (!success) {
      return res.status(404).json({ error: 'Recipient not found' });
    }
    res.json({ success: true });
  } catch (error) {
    console.error('Error removing executive report recipient:', error);
    res.status(500).json({ error: error.message });
  }
});

// Get executive report preview (data only)
router.get('/executive-report/preview', async (req, res) => {
  try {
    const data = await executiveReportService.buildExecutiveReport();
    res.json(data);
  } catch (error) {
    console.error('Error building executive report preview:', error);
    res.status(500).json({ error: error.message });
  }
});

// Send executive report to all recipients
router.post('/executive-report/send', async (req, res) => {
  try {
    const results = await executiveReportService.sendExecutiveReport();
    res.json({
      success: true,
      message: `Executive report sent: ${results.sent} successful, ${results.failed} failed`,
      ...results
    });
  } catch (error) {
...
// Send test executive report
router.post('/executive-report/send-test', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) {
      return res.status(400).json({ error: 'Email is required' });
    }
    const result = await executiveReportService.sendTestExecutiveReport(email);
    res.json(result);
  } catch (error) {
    console.error('Error sending test executive report:', error);
    res.status(500).json({ error: error.message });
  }
});
